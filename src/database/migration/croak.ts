...
export async function up(db: Kysely<Database>): Promise<void> {
  await db.schema
    ...
    .addColumn("created_date", "timestamp", (col) => col.defaultTo(sql`now()`).notNull())
    .addColumn("updated_date", "timestamp", (col) => col.defaultTo(sql`now()`).notNull())
    .execute();
  ...
  await db.schema
    ...
    .addColumn("posted_date", "timestamp", (col) => col.defaultTo(sql`now()`).notNull())
