...
export type Thread = (db: Kysely<Database>) => (threadId: number, reverse: boolean, offsetCursor: number, limit: number) => Promise<Croak[]>;
export const thread: Thread =
  (db) =>
  (threadId, reverse, offsetCursor, limit) =>
  complementCroak(db, () => getCroaks(db)(threadId, reverse, offsetCursor, limit));

type GetCroaks = (db: Kysely<Database>) => (threadId: number, reverse: boolean, offsetCursor: number, limit: number) => Promise<CroakSimple[]>;
