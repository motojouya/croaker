...
export type Search = (db: Kysely<Database>) => (search: string, reverse: boolean, offsetCursor: number, limit: number) => Promise<Croak[]>;
export const search: Search =
  (db) =>
  (search, reverse, offsetCursor, limit) =>
  complementCroak(db, () => getCroaks(db)(search, reverse, offsetCursor, limit));

type GetCroaks = (db: Kysely<Database>) => (search: string, reverse: boolean, offsetCursor: number, limit: number) => Promise<CroakSimple[]>;
      ...
      (eb) => eb
        ...
        .select((ebs) => ([
          ...
          ebs.fn.sum(
            ebs
              .case()
              .when('subk.contents', 'like', `%${search}%`)
              .then(1)
              .else(0)
              .end()
          ).as('exist_count'),
